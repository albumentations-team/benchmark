"""AlbumentationsX implementations of transforms for images in custom format."""
...
class _ColorizeFromRgb:
    ...
    def __call__(self, **kwargs: Any) -> dict[str, Any]:
        ...
        if gray.ndim == 2:
            gray = gray[..., None]
        return {"image": self._colorize(image=gray)["image"]}
...
def __call__(transform: Any, image: Any) -> Any:  # noqa: N807
    ...
    if isinstance(transform, A.CopyAndPaste):
        return np.ascontiguousarray(transform(image=image, copy_paste_metadata=[])["image"])
