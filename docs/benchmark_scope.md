...
| --- | --- | --- | --- |
| 9-channel GPU micro | `configs/paper/prod_g2_9ch_micro_gpu.yaml` | Published from `results/published/paper-9ch-micro-gpu-g2-standard-16-2026-05-10/`. Kornia has 34 ok rows and 4 unsupported CUDA rows; TorchVision has 21 ok rows. | Keep as the 9-channel GPU micro result set. |
| 9-channel CPU micro | `configs/paper/prod_c4_9ch_micro_cpu.yaml` | Published from `results/published/paper-9ch-micro-c4-standard-16-2026-05-14/`. AlbumentationsX has 41 ok rows, TorchVision has 21 ok rows plus one early-stopped row, and Kornia has 36 ok rows plus four early-stopped rows. | Keep as the 9-channel CPU micro result set. |
| 9-channel CPU DataLoader | `configs/paper/prod_c4_9ch_dataloader_cpu.yaml` plus Kornia highmem rerun | AlbumentationsX and TorchVision are published from `results/published/paper-9ch-dataloader-memory-c4-standard-16-2026-05-10/`; Kornia is published from `results/published/paper-9ch-dataloader-memory-c4-highmem-16-2026-05-11/`. Kornia has 40 ok rows. | Keep the merged public CPU DataLoader result set and preserve the highmem provenance for Kornia. |
| 9-channel GPU DataLoader | `configs/paper/prod_g2_9ch_dataloader_gpu.yaml` | Published from `results/published/paper-9ch-dataloader-gpu-decode-g2-standard-16-2026-05-11/`. TorchVision has 21 ok rows; Kornia has 37 ok rows plus one unsupported row. | Keep as the 9-channel GPU DataLoader result set. |
| Video CPU micro | `configs/paper/prod_c4_video_micro_cpu.yaml` | Published from `results/published/paper-video-micro-c4-standard-16-2026-05-14/`. AlbumentationsX has 51 ok rows, TorchVision has 26 ok rows, and Kornia has 40 ok rows plus seven unsupported rows and one early-stopped row after excluding confirmed crash-only rows. | Keep as the video CPU micro result set. |
...
| Video DALI GPU DataLoader | `configs/paper/prod_g2_video_dataloader_dali.yaml` | Published from GCS run `a22bac0b4a9946e3aaf25278fa206966`. DALI native video pipeline has 21 ok rows and 29 unsupported rows. | Keep as a separately labeled native DALI pipeline baseline with `decode_dataloader_augment` scope because video read/decode is inside the timed DALI graph. |
